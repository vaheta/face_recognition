...
void ftt_reader (string filename, pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud, pcl::PointXYZRGB nose_tip) {
	...
	if (myfile.is_open()) {
		while (getline(myfile, line)) {
			...
			double x,y,z;
			int t2;
			char t1[100],poz[100],sx[10],sy[10],sz[10];
			string flag = " 3D ";
			string start = "(";
			string end = ")";
			// if (sscanf(line.c_str(), "%[^'3']3D (%[^' '] %[^' '] %[^')'])%[]", t1,sx,sy,sz,poz) == 1) {
			if (line.find(flag) != string::npos) {
				...
				if (line.find("nose-1") != string::npos && 
					line.find("nose-10") == string::npos && 
					line.find("nose-11") == string::npos &&
					line.find("nose-12") == string::npos) {
					nose_tip.x = x;
					nose_tip.y = y;
					nose_tip.z = z;	
				}
